...
bool Position::position_is_legal()
{
    ...
    uint64_t king =
        side_to_move == Color::Black ? white_kings : black_kings;
...
std::vector<Move> Position::generate_pawn_moves()
{
    return generate_pseudo_pawn_moves();
    // clean them up afterwards
}

std::vector<Move> Position::generate_pseudo_pawn_moves()
{
    std::vector<Move> res;
...
std::vector<Move> Position::generate_knight_moves()
{
    return generate_pseudo_knight_moves();
    // clean them up afterwards
}

std::vector<Move> Position::generate_pseudo_knight_moves()
{
    // knight mask
    std::vector<uint64_t> knight_mask(64, 0ULL);
    knight_mask[0] = 0x0000000000020400ULL;
...
std::vector<Move> Position::generate_rook_moves()
{
    return generate_pseudo_rook_moves();
    // clean them up afterwards
}

void Position::rook_castling_right_helper(Move &m)
{
...
std::vector<Move> Position::generate_pseudo_rook_moves()
{
    std::vector<Move> res;
...
std::vector<Move> Position::generate_bishop_moves()
{
    return generate_pseudo_bishop_moves();
    // clean them up afterwards
}

std::vector<Move> Position::generate_pseudo_bishop_moves()
{
    std::vector<Move> res;
...
std::vector<Move> Position::generate_queen_moves()
{
    return generate_pseudo_queen_moves();
    // clean them up afterwards
}

std::vector<Move> Position::generate_pseudo_queen_moves()
{
    std::vector<Move> res;
...
std::vector<Move> Position::generate_king_moves()
{
    return generate_pseudo_king_moves();
    // clean them up afterwards
}

std::vector<Move> Position::generate_pseudo_king_moves()
{
...
void print_full_board(Position &p)
{
    ...
    for (int rank = 8; rank >= 1; --rank)
    {
        for (char file = 'a'; file <= 'h'; ++file)
        {
            ...
            char res = color == Color::White ? 'W' : color == Color::Black ? 'B'
                                                                           : '.';
            std::cout << res << " ";
        }
        std::cout << std::endl;
