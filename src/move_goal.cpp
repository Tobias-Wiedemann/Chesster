...
// bits 7 to 12
uint32_t Move::get_to() const { return (m & 0xFFFFFF03F) >> 6; }
void Move::set_to(uint32_t to) { m = (m & 0xFC0) | to << 6; }
...
bool Move::previous_en_passant_square_existed() const {
  return (m & 0x40000000) != 0;
}
// bits 19 to 24
uint32_t Move::get_previous_en_passant_square() const {
  return (m & 0xFC0000) >> 19;
}
void Move::set_previous_en_passant_square(uint32_t previous_en_passant_square) {
  m = (m & 0xFF03FFFF) | (previous_en_passant_square << 19);
  m = m | 0x40000000;
}
