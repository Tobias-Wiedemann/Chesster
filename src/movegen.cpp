...
std::vector<Move> MoveGenerator::generate_pseudo_pawn_moves()
{
    ...
    if (p.side_to_move == Color::White)
    {
        ...
        double_pushable_pawns = (((pushable_pawns & white_pawns_starting_mask) << 16) & p.empty_squares) >> 16;
    }
    else
    {
        pawns = p.black_pawns;
        ...
        pushable_pawns = (pushed_pawns & p.empty_squares) << 8;
        double_pushable_pawns = (((pushable_pawns & black_pawns_starting_mask) >> 16) & p.empty_squares) << 16;
    }
    ...
    while (pawns)
    {
        ...
        if (index % 8 < 7)
        {
            ...
            int attacked_index = p.side_to_move == Color::White ? index + 9 : index - 7;
        ...
        if (index % 8 > 0)
        {
            ...
            int attacked_index = p.side_to_move == Color::White ? index + 7 : index - 9;
            if (p.piece_table[attacked_index] != Piece::Empty &&
                p.color_table[attacked_index] != p.side_to_move)
            {
                Move m(index, attacked_index, Move_Type::Capture);
